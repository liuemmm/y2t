...
export function generateInterface(
  ...
  let { apiTemplate } = config
  if (!config.apiTemplate) {
    apiTemplate = fs
      .readFileSync(path.join(__dirname, '../templates/apiTemplate/api.tpl'))
      .toString()
  }

  if (!(projectId in config.projectMapping)) {
    throw new Error(
      `生成失败，项目：${projectName}，ID：${projectId} 未配置 projectMapping`,
    )
  }
  ...
  const apiFilePath = path.resolve(
    `${config.outDir}/${underlineToHump(projectName)}`,
  )
  // api文件的完整文件名
  ...
  if (fs.existsSync(apiFileName)) {
    ...
    // 判断模块是否存在了
    const interfaceExp = new RegExp(
      `\\/\\/\\s*#region\\s*interface:${modularId}`,
    )
    const isExistModule = interfaceExp.test(originContent)
    // 已存在该module，根据#region interface 标识去修改
    if (isExistModule) {
      const reg = new RegExp(
        `\\/\\/\\s*#region\\s*interface:${modularId}[\\s\\S]*\\/\\/\\s*#endregion\\s*interface:${modularId}`,
      )
    ...
    apiContent = apiTemplate
      ...
      .replace(
        '{Wrapper}',
        wrapper ? `interface IResponseWrapper<T> ${wrapper} \r\n` : '',
      ) // 配置统一请求返回体
      .replace(
        '{Mock}',
        `${config.originUrl}/mock/${projectId}${basePath || ''}`,
      ) // 替换mock路径
...
function generateApiFunction(
  ...
  let { apiBodyTemplate } = config
  if (!config.apiBodyTemplate) {
    apiBodyTemplate = fs
      .readFileSync(path.join(__dirname, '../templates/apiTemplate/body.tpl'))
      .toString()
  }
  // 获取命名空间名
  ...
  // 参数Query
  const query =
    api.detail.query && api.detail.query.length > 0
      ? `query: ${getQueryPath(namespaceName, api)},\n`
      : ''
  // 参数Body
  const body =
    isNeedBody && api.detail.body
      ? `body: ${getBodyPath(namespaceName, api)},\n`
      : ''
  // 返回的类型
  ...
  // 请求Query
  const queryParam =
    api.detail.query && api.detail.query.length > 0 ? 'params: query' : ''
  const bodyContent = apiBodyTemplate
    .replace('{Description}', description)
    .replace(
      '{InterfaceName}',
      `${interfaceName}${response === 'T' ? '<T = any>' : ''}`,
    )
