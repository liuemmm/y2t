 ...
 * @return {*}
 */
export async function getModularList(
  projectId: number,
): Promise<IModularLiatResponse> {
  return new Promise((resolve) => {
    ...
    http
      ...
      .then(async (modularReq) => {
        // 抽取关键数据
        const modularList: IListItem[] =
          modularReq?.data?.data?.cat?.map((e: any) => {
            return {
              name: e.name,
              id: e._id,
            }
...
// 获取菜单ID
export async function getModular(
  projectId: number,
): Promise<IModularResponse[]> {
  return new Promise(async (resolve) => {
    ...
    // 获取选择的模块
    const modulars = modularList.modularList.filter((e) =>
      modularNames.includes(e.name),
    )
